...
export class RegisterComponent {
  ...
  onSubmit() {
    ...
    const newUser: UserSignupDTO = {
      ...
      displayName: this.nom, // Mapping to displayName
      password: this.password,
      email: this.email,
      firstName: this.prenom, // Mapping to firstName
      lastName: '', // Assuming you might allow this to be optional
      numtel: this.numtel,
    ...
    this.userService.register(newUser).subscribe({
      next: (response) => {
        ...
        this.router.navigate(['/profile']); // Navigate to profile page on success
      },
      error: (err) => {
