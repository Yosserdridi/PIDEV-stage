...
@Component({
  ...
  styleUrls: ['./list-rejected.component.css']
})
export class ListRejectedComponent {
  posts: Post[] = [];
    ...
    loadPosts(): void {
      this.postService.getAllPosts().subscribe(
        (data) => {
          ...
          this.filteredPosts = this.posts.filter(post => post.status === 'Rejected'); // Initialize filtered posts with all posts
